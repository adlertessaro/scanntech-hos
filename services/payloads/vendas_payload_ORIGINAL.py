...
def buscar_venda_original(venda_dev, empresa, cupom):
    ...
    cur = conn.cursor()
    try:
        # Busca a venda mais recente com o mesmo número de cupom e empresa
        cur.execute("""
...
def montar_payload_da_venda(venda, empresa, config, estacao, is_devolucao=False, cupom=None, lancamen=None, valor_total=None, force_as_sale=False, debug_mode=False):
    ...
    try:
        ...
        # TRatamento para itens com valor zero ou negativo (pode acontecer em devoluções ou erros de cadastro)
        if detalhes:
            detalles_filtrados = []
        ...
        total_final_calculado = round(sum(d['importe'] for d in detalhes), 2)
        ...
        payload = {
            ...
            "total": total_final_calculado,
            "codigoMoneda": "986",
            "cotizacion": 1.00,
            "descuentoTotal": round(desconto_calculado_seguro, 2),
            "recargoTotal": round(recargo_calculado_seguro, 2),
        ...
        if log_avancado:
            ...
            # Usamos a variável que acabamos de criar para o log também
            logging.info(f"\n   ✅ VALIDAÇÃO FINAL:")
            logging.info(f"      Total do cupom (enviado): R$ {total_final_calculado:.2f}")
            logging.info(f"      Soma itens: R$ {total_final_calculado:.2f}")
