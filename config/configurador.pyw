...
class ConfiguradorApp:
    ...
    def __init__(self, master):
        ...
        self.campos_gerais = {
            ...
            "Log Avançado": tk.BooleanVar(value=False),
            "Carga Inicial": tk.BooleanVar(value=False),
    ...
    def criar_widgets_gerais(self):
        ...
        check_log_avancado = ttk.Checkbutton(
            integracao_frame,
            text="Log Avançado (Debug Detalhado)",
            variable=self.campos_gerais["Log Avançado"],
            bootstyle="round-toggle"
        )
        check_log_avancado.pack(anchor="w", pady=5)
        ToolTip(
            check_log_avancado, 
            text="Ativa logs detalhados: JSON completo dos payloads, validações de soma, detalhes de pagamentos e fechamentos.", 
            bootstyle="inverse"
        )
        check_carga = ttk.Checkbutton(
            integracao_frame,
            text="Executar Carga Inicial (Respeitar Datas Fixas)",
            variable=self.campos_gerais["Carga Inicial"],
            bootstyle="round-toggle"
        )
        check_carga.pack(anchor="w", pady=5)
        ToolTip(check_carga, text="Se ativado, o próximo ciclo usará as datas de início abaixo. Após o envio, será desativado automaticamente.", bootstyle="inverse")
    ...
    def carregar_configuracoes(self):
        try:
            ...
            self.campos_gerais["Log Avançado"].set(config_geral.get("log_avancado", "false").lower() == "true")
            self.campos_gerais["Carga Inicial"].set(config_geral.get("carga_inicial", "false").lower() == "true")
    ...
    def salvar_config(self):
        ...
        parser.set("SCANNTECH_GERAL", "log_avancado", str(self.campos_gerais["Log Avançado"].get()))
        parser.set("SCANNTECH_GERAL", "carga_inicial", str(self.campos_gerais["Carga Inicial"].get()).lower())
